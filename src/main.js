...
function filtrado() {
    ...
    const filtradoGenero = filterGender(personaje, selectedOption)
    crearElementos(filtradoGenero)
    }

    crearElementos(personaje)
    const select2 = document.getElementById('filtrar');
    select.addEventListener('change', filtrado2)
    function filtrado2() {
        let selectedOption = this.options[select.selectedIndex].value;
        addArticle.innerHTML = ""
        const filtradoCasas = filterHouse(personaje, selectedOption)
        crearElementos(filtradoCasas)
        }
